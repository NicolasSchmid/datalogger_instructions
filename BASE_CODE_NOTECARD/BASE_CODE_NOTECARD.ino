...
void send_data_overGSM(){
  ...
  int nb_lines_to_send =nb_meas_sent_at_once; 
  ...
  */
  String data_matrix[nb_lines_to_send][2+sensor.get_nb_values()]; //the "2 + ..." is there because we also put Bootcount and DateTime in our matrix
  int counter=0;
  ...
  for (int i =0; i<nb_lines_to_send;i++) {
    ...
    for(int j=0;j<2+sensor.get_nb_values();j++){        
  ...
  if(synchronize_notecard()){ //connect the notecard
    ...
    for(int i=0; i<sensor.get_nb_values();i++){
      ...
      for(int j=0; j<nb_lines_to_send;j++){
        ...
        JAddStringToObject(sample, "epoch", String(datetime_stored.unixtime()-3600).c_str()); //we remove one hour from the time store since the notehub needs the GMT+0 time and we store the GMT+1 time (swiss winter time)
      }
    }
